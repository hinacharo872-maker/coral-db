...
function formatPositiveNumber(value) {
  ...
  return numericValue.toLocaleString('ja-JP', { maximumFractionDigits: 2 })
}

export function formatTankDimensions(tank = {}) {
  const dimensions = [
    { label: '幅', value: formatPositiveNumber(tank.tank_width_cm) },
    { label: '奥行', value: formatPositiveNumber(tank.tank_depth_cm) },
    { label: '高さ', value: formatPositiveNumber(tank.tank_height_cm) },
  ].filter(dimension => dimension.value)

  if (dimensions.length === 3) {
    return `${dimensions.map(dimension => dimension.value).join(' × ')} cm`
  }
  return dimensions.map(dimension => `${dimension.label} ${dimension.value} cm`).join(' / ')
}

export function formatTankVolume(value) {
  const volume = formatPositiveNumber(value)
...
export function buildLiteEnvironmentParts(tank = {}) {
  ...
  const dimensions = formatTankDimensions(tank)
  const volume = formatTankVolume(tank.tank_volume_liters)
  const parts = []

  if (dimensions || volume || tank.filtration_method) {
    parts.push({
      key: 'tank',
      label: '水槽本体',
      value: dimensions || volume || '水槽',
      details: [
        dimensions && `水槽サイズ：${dimensions}`,
        volume && `実水量：${volume}`,
...
export function hasLiteEnvironment(tank) {
  return Boolean(
    formatTankDimensions(tank)
    || formatTankVolume(tank?.tank_volume_liters)
    || tank?.ph != null
    || tank?.salt_mix_name
