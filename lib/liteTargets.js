...
export const LITE_EMERGENCY_THRESHOLDS = Object.freeze({
  temperature_c: { below: 22, above: 30 },
  kh_dkh: { below: 5 },
  salinity_sg: { below: 1.020, above: 1.030 },
})
...
export function getLiteEmergencyDirection(parameterKey, value) {
  if (value == null || value === '' || !Number.isFinite(Number(value))) return null

  const threshold = LITE_EMERGENCY_THRESHOLDS[parameterKey]
  if (!threshold) return null

  const numericValue = Number(value)
  if (threshold.below != null && numericValue < threshold.below) return 'low'
  if (threshold.above != null && numericValue > threshold.above) return 'high'
  return null
}
